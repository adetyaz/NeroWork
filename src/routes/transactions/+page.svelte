...
let contractAddress = $state('0x6da24cb091a69bc3026b233dbdd146ceb3b72727'); // NeroInvoice contract address
let badgeContractAddress = $state('0x69f87d3d588c049ace05975212c6ed272e5c1fa9'); // BadgeNFT contract address
...
let selectedMilestone = $state(milestones[0]);
let imageUrl = $state(selectedMilestone.imageUrl);
...
<div class="max-w-xl mx-auto p-8 bg-white rounded shadow mt-10">
  ...
  <button class="bg-blue-600 text-white px-4 py-2 rounded" onclick={mintInvoice} disabled={loading}>
  ...
  <div class="mb-4">
    ...
    <select class="w-full border rounded px-3 py-2" onchange={handleMilestoneChange}>
  ...
  <button class="bg-blue-600 text-white px-4 py-2 rounded" onclick={mintBadge} disabled={loading}>
